...
export const SnappingScrollerSelector = ({closestCardDetails, selection, onSelection}) => {
    const [clipPath, setClipPath] = useState(null);
    const lineOfFocusRef = useRef(null);

    function createCardClip(card, line){//Only works for uniform and perfect circles of borderRadius
        if(!card) return;
        ...
        const {centerY: lineCenterY, height: lineHeight, width: lineWidth} = lineOfFocusDimensions;
  ...
  return (
    <FocusLine $clipPath={clipPath} ref={lineOfFocusRef}></FocusLine>
